...
public sealed class WorkforceOperationsController(IWorkforceOperationsService service) : ControllerBase
{
    ...
    [HttpGet("timesheets")] public Task<PagedResult<TimesheetDto>> Timesheets([FromQuery] int page = 1, [FromQuery] int pageSize = 25, [FromQuery] string? search = null, [FromQuery] Guid? employeeId = null, [FromQuery] WorkflowStatus? status = null, CancellationToken ct = default) => service.SearchTimesheetsAsync(new(page, pageSize, search), employeeId, status, ct);
