...
public sealed class SelfServiceController(ISelfService service) : ControllerBase
{
    ...
    [HttpPut("attendance/{id:guid}/location")] public async Task<IActionResult> AttendanceLocation(Guid id, AttendanceLocationRequest request, CancellationToken ct) { await service.AttachAttendanceLocationAsync(id, request, ct); return NoContent(); }
