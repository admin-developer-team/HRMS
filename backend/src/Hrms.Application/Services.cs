...
public interface IAttendanceService
{
    ...
    Task AttachLocationAsync(Guid attendanceId, Guid employeeId, AttendanceLocationRequest request, CancellationToken cancellationToken);
...
public interface ISelfService
{
    ...
    Task AttachAttendanceLocationAsync(Guid attendanceId, AttendanceLocationRequest request, CancellationToken cancellationToken);
    ...
    IEmailQueue? emailQueue = null) : ITenantService
{
    public async Task<TenantDto> CreateAsync(CreateTenantRequest request, CancellationToken ct)
    {
        ...
        {
            TenantId = tenant.Id, PlanCode = "trial", EmployeeLimit = Math.Clamp(request.EmployeeLimit, 1, 10),
    ...
    public async Task<TenantDto> UpdateAsync(Guid id, UpdateTenantRequest r, CancellationToken ct)
    {
        ...
        try
        {
            ...
            var followTrialEnd = tenant.Status == TenantStatus.Trial && subscription.PlanCode == "trial"
                && subscription.EndsAt == tenant.TrialEndsAt && r.SubscriptionEndsAt == subscription.EndsAt;
            ...
            subscription.StartsAt = r.SubscriptionStartsAt.ToUniversalTime();
            subscription.EndsAt = followTrialEnd && r.Status == TenantStatus.Trial && r.PlanCode.Equals("trial", StringComparison.OrdinalIgnoreCase)
                ? tenant.TrialEndsAt : r.SubscriptionEndsAt?.ToUniversalTime();
            subscription.IsActive = r.SubscriptionActive;
    ...
    private static TenantDto Map(Tenant tenant, TenantSubscription? subscription) => new(
        ...
        subscription?.PlanCode == "trial" ? Math.Min(subscription.EmployeeLimit, 10) : subscription?.EmployeeLimit ?? 0,
        tenant.TrialEndsAt, subscription?.PlanCode ?? "unassigned",
...
public sealed class EmployeeService(IRepository<Employee> employees, IRepository<UserAccount> users, IRepository<RefreshToken> refreshTokens, IRepository<TenantSubscription> subscriptions, IRepository<AuditLog> auditLogs, ICurrentTenant tenant, ICurrentUser actor, IUnitOfWork unitOfWork, IRepository<Department> departments, IRepository<Designation> designations, IRepository<Location> locations, IEmailQueue? emailQueue = null) : ServiceBase(tenant), IEmployeeService
{
    public async Task<EmployeeDto> CreateAsync(CreateEmployeeRequest r, CancellationToken ct)
    {
        ...
        ValidateOfficeHours(r.OfficeStartsAt, r.OfficeEndsAt);
        ...
        var employeeLimit = sub?.PlanCode == "trial" ? Math.Min(sub.EmployeeLimit, 10) : sub?.EmployeeLimit;
        if (employeeLimit.HasValue && await employees.CountAsync(x => x.Status == EmploymentStatus.Active || x.Status == EmploymentStatus.Probation || x.Status == EmploymentStatus.NoticePeriod, ct) >= employeeLimit.Value)
            throw new DomainException(sub?.PlanCode == "trial" ? "The free trial allows up to 10 employees." : "The subscription employee limit has been reached.");
        var e = new Employee
        {
            ...
            OfficeStartsAt = r.OfficeStartsAt, OfficeEndsAt = r.OfficeEndsAt
        };
    ...
    public async Task<PagedResult<EmployeeDto>> SearchAsync(PagedRequest r, EmploymentStatus? status, Guid? departmentId, CancellationToken ct)
    {
        ...
        System.Linq.Expressions.Expression<Func<Employee, bool>> predicate = x =>
            ...
            (string.IsNullOrEmpty(q) || x.EmployeeNumber.ToLower().Contains(q) || x.FirstName.ToLower().Contains(q) || x.LastName.ToLower().Contains(q) || (x.FirstName + " " + x.LastName).ToLower().Contains(q) || x.WorkEmail.ToLower().Contains(q));
    ...
    public async Task<EmployeeDto> UpdateAsync(Guid id, UpdateEmployeeRequest r, CancellationToken ct)
    {
        ...
        e.OfficeStartsAt = r.OfficeStartsAt; e.OfficeEndsAt = r.OfficeEndsAt;
    ...
    private static void ValidateOfficeHours(TimeOnly? start, TimeOnly? end)
    {
        if (start.HasValue != end.HasValue) throw new DomainException("Set both employee office start and end times, or leave both blank to use company hours.");
        if (start.HasValue) _ = AttendanceCalendar.DurationMinutes(start.Value, end!.Value);
    }
    private static EmployeeDto Map(Employee e) => new(e.Id, e.EmployeeNumber, e.FirstName, e.LastName, e.FullName, e.WorkEmail, e.Phone, e.HireDate, e.Status, e.EmploymentType, e.DepartmentId, e.DesignationId, e.LocationId, e.ManagerId, e.BaseSalary, e.SalaryCurrency, e.UserId, e.Version, e.OfficeStartsAt, e.OfficeEndsAt);
}
...
{
    public async Task AttachLocationAsync(Guid attendanceId, Guid employeeId, AttendanceLocationRequest r, CancellationToken ct)
    {
        var row = await records.GetByIdAsync(attendanceId, ct) ?? throw new KeyNotFoundException("Attendance session not found.");
        if (row.EmployeeId != employeeId) throw new UnauthorizedAccessException("You can only attach your own attendance location.");
        if (r.Latitude is < -90 or > 90 || r.Longitude is < -180 or > 180 || r.AccuracyMeters < 0)
            throw new DomainException("Location coordinates are invalid.");
        var now = DateTimeOffset.UtcNow;
        if (r.Action == "clock-in")
        {
            if (row.ClockedInAt is null || now - row.ClockedInAt > TimeSpan.FromMinutes(2) || row.ClockInLatitude.HasValue)
                throw new DomainException("Check-in location can only be attached shortly after check-in.");
            row.ClockInLatitude = r.Latitude; row.ClockInLongitude = r.Longitude; row.ClockInAccuracyMeters = r.AccuracyMeters;
        }
        else if (r.Action == "clock-out")
        {
            if (row.ClockedOutAt is null || now - row.ClockedOutAt > TimeSpan.FromMinutes(2) || row.ClockOutLatitude.HasValue)
                throw new DomainException("Check-out location can only be attached shortly after check-out.");
            row.ClockOutLatitude = r.Latitude; row.ClockOutLongitude = r.Longitude; row.ClockOutAccuracyMeters = r.AccuracyMeters;
        }
        else throw new DomainException("Attendance action is invalid.");
        await unitOfWork.SaveChangesAsync(ct);
    }
    ...
    public async Task<AttendanceDto> ClockInAsync(ClockRequest r, CancellationToken ct)
    {
        ...
        var company = await tenants.GetByIdAsync(TenantId, ct);
        var scheduledStart = employee.OfficeStartsAt ?? policy.OfficeStartsAt;
        var scheduledEnd = employee.OfficeEndsAt ?? policy.OfficeEndsAt;
        var date = AttendanceCalendar.WorkDate(at, AttendanceCalendar.Zone(company?.TimeZone), scheduledStart, scheduledEnd);
        ...
        var x = new AttendanceRecord { TenantId = TenantId, EmployeeId = r.EmployeeId, WorkDate = date, ClockedInAt = at, Source = source, Notes = notes, ScheduledStartAt = scheduledStart, ScheduledEndAt = scheduledEnd, RequiredMinutes = AttendanceCalendar.DurationMinutes(scheduledStart, scheduledEnd), LateGraceMinutes = policy.LateGraceMinutes, EarlyDepartureGraceMinutes = policy.EarlyDepartureGraceMinutes, ClockInLatitude = r.Latitude, ClockInLongitude = r.Longitude, ClockInAccuracyMeters = r.AccuracyMeters, ClockInAddress = r.Address, ClockInIpAddress = r.IpAddress, ClockInUserAgent = r.UserAgent };
        ...
        foreach (var person in people)
        {
            ...
            for (var date = firstDate; date <= end && date <= today && (!person.TerminationDate.HasValue || date <= person.TerminationDate.Value); date = date.AddDays(1))
            {
                ...
                if (daySessions.Length == 0)
                {
                    ...
                    var scheduledMinutes = person.OfficeStartsAt.HasValue && person.OfficeEndsAt.HasValue
                        ? AttendanceCalendar.DurationMinutes(person.OfficeStartsAt.Value, person.OfficeEndsAt.Value) : policy.RequiredMinutesPerDay;
                    var scheduledHours = holiday || onLeave ? 0 : Math.Round(scheduledMinutes / 60m, 2);
                ...
                var scheduledStart = snapshot.ScheduledStartAt ?? person.OfficeStartsAt ?? policy.OfficeStartsAt;
                var scheduledEnd = snapshot.ScheduledEndAt ?? person.OfficeEndsAt ?? policy.OfficeEndsAt;
                ...
                var requiredMinutes = snapshot.RequiredMinutes ?? AttendanceCalendar.DurationMinutes(scheduledStart, scheduledEnd);
                ...
                if (policy.RequireLocationCapture && daySessions.Any(x => x.ClockInLatitude is null || (x.ClockedOutAt.HasValue && x.ClockOutLatitude is null)))
                    issues.Add("Location unavailable");
...
public sealed class WorkforceOperationsService(IRepository<Shift> shifts, IRepository<Holiday> holidays, IRepository<TimesheetEntry> timesheets, IRepository<EmployeeDocument> documents, IRepository<Announcement> announcements, IRepository<Employee> employees, IRepository<Location> locations, IRepository<HolidaySelection> selections, ICurrentTenant tenant, IUnitOfWork unitOfWork, INotificationService notifications) : ServiceBase(tenant), IWorkforceOperationsService
{
    ...
    public async Task<PagedResult<TimesheetDto>> SearchTimesheetsAsync(PagedRequest r, Guid? employeeId, WorkflowStatus? status, CancellationToken ct) { var query = r.Search?.Trim().ToLowerInvariant(); System.Linq.Expressions.Expression<Func<TimesheetEntry, bool>> p = x => (!employeeId.HasValue || x.EmployeeId == employeeId) && (!status.HasValue || x.Status == status) && (string.IsNullOrEmpty(query) || x.Description.ToLower().Contains(query) || (x.ProjectCode != null && x.ProjectCode.ToLower().Contains(query))); var total = await timesheets.CountAsync(p, ct); var rows = await timesheets.ListAsync(p, q => q.OrderByDescending(x => x.WorkDate), r.Skip, r.SafePageSize, ct); return new(rows.Select(Map).ToArray(), r.SafePage, r.SafePageSize, total); }
...
public sealed class ExpenseService(IRepository<ExpenseClaim> expenses, IRepository<Employee> employees, IRepository<StoredDocument> documents, ICurrentTenant tenant, ICurrentUser user, IUnitOfWork unitOfWork, INotificationService notifications) : ServiceBase(tenant), IExpenseService
{
    ...
    public async Task<PagedResult<ExpenseDto>> SearchAsync(PagedRequest r, Guid? employeeId, ExpenseStatus? status, CancellationToken ct) { var query = r.Search?.Trim().ToLowerInvariant(); System.Linq.Expressions.Expression<Func<ExpenseClaim, bool>> p = x => (!employeeId.HasValue || x.EmployeeId == employeeId) && (!status.HasValue || x.Status == status) && (string.IsNullOrEmpty(query) || x.ClaimNumber.ToLower().Contains(query) || x.Category.ToLower().Contains(query) || x.Description.ToLower().Contains(query)); var total = await expenses.CountAsync(p, ct); var rows = await expenses.ListAsync(p, q => q.OrderByDescending(x => x.ExpenseDate), r.Skip, r.SafePageSize, ct); return new(rows.Select(Map).ToArray(), r.SafePage, r.SafePageSize, total); }
